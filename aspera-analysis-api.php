...
/**
 * Welke theme-optie bij welk breakpoint-slot hoort. Alleen voor de detail-tekst
 * van `responsive_breakpoint_width_mismatch`, zodat de melding benoemt waarnaar
 * de waarde zich hoort te richten in plaats van alleen een getal te noemen.
 */
const ASPERA_RESPONSIVE_SLOT_LABELS = [
...
/**
 * Ruwe breakpoint-breedte -> px als int, null wanneer het breakpoint UIT staat,
 * false bij onzin ("px", "pxpx").
 *
 * "Uit" heeft drie verschijningsvormen: attribuut afwezig, lege waarde, of "0px".
 * Die laatste is geen randgeval maar de normale uit-stand: de control is een slider
 * met een minimum, dus de gebruiker kan het veld niet leegmaken en zet hem op 0 om
 * het breakpoint niet te gebruiken.
 *
 * Staat als losse functie en niet als closure in de scanner, zodat de fix-action
 * hetzelfde criterium hanteert als de detectie (zie handboek sectie 8).
 *
 * @return int|null|false
 */
function aspera_responsive_px( $raw ) {
    if ( $raw === null ) {
        return null;
    }
    $raw = trim( (string) $raw );
...
/**
 * Zet één attribuut in een shortcode-openingstag: vervangt de waarde wanneer het
 * attribuut al bestaat, en voegt het anders toe vlak voor de sluitende bracket.
 * Alle overige attributen en hun volgorde blijven ongemoeid.
 */
function aspera_sc_set_attr( string $sc, string $name, string $value ): string {
    $pattern = '/\b' . preg_quote( $name, '/' ) . '="[^"]*"/';
    if ( preg_match( $pattern, $sc ) ) {
        return preg_replace( $pattern, $name . '="' . $value . '"', $sc, 1 );
    }
    return rtrim( substr( $sc, 0, -1 ) ) . ' ' . $name . '="' . $value . '"]';
}

/**
 * Laptops-, tablets- en mobiles-breakpoint uit de Impreza theme options, in px.
 *
 ...
 * verwachting te stellen en is zwijgen beter dan onjuist melden. Alle drie de
 * waarden zijn vereist; ontbreekt er een, dan is de verwachting voor het
 * bijbehorende slot niet te stellen en vervalt de hele regel.
 */
function aspera_theme_breakpoints(): ?array {
    ...
    if ( ! is_array( $opts )
        || ! isset( $opts['laptops_breakpoint'], $opts['tablets_breakpoint'], $opts['mobiles_breakpoint'] ) ) {
        return $cached = null;
    }
    $laptops = (int) $opts['laptops_breakpoint'];
    $tablets = (int) $opts['tablets_breakpoint'];
    $mobiles = (int) $opts['mobiles_breakpoint'];
    if ( $laptops <= 0 || $tablets <= 0 || $mobiles <= 0 ) {
        return $cached = null;
    }
    return $cached = [ 'laptops' => $laptops, 'tablets' => $tablets, 'mobiles' => $mobiles ];
}
...
function aspera_wpb_validate_post( WP_Post $post ): array {
    ...
    foreach ( $matches as $m ) {
        ...
        // voor slot 3. Een niet-gezet attribuut valt daarop terug en voldoet dus
...
function aspera_validate_color_value( string $value, string $attr_name, array $whitelist, array $hex_map = [] ): ?array {
    ...
    if ( isset( $value[0] ) && $value[0] === '_' ) {
        ...
        // Custom Color reference: _cc1, _cc2, etc. — vóór hex-check want cc+cijfer matcht hex-regex
        if ( preg_match( '/^cc\d+$/', $var_name ) ) {
            return [
                'rule'     => 'deprecated_custom_var',
                'detail'   => $attr_name . '="' . $value . '" — Custom Color referentie; vervang door een framework CSS var',
                'severity' => 'warning',
            ];
        }
...
function aspera_site_health_test(): array {
    ...
    $cat_labels = [
        ...
        'options_orphaned' => 'Verweesde Options',
        'options_config'   => 'Option pages — configuratie',
...
function aspera_admin_page_render(): void {
    ...
    echo '<p class="description">Diagnostisch hulpmiddel voor WordPress-sites</p>';
...
add_action( 'wp_ajax_aspera_apply_fix', function () {
    ...
    switch ( $action ) {
        ...
        // Zet de zes responsive-breakpoint-attributen van één lijst-element goed.
        //
        // De browser levert alleen post_id en de shortcode die hersteld moet worden;
        // de nieuwe waarden worden hier opnieuw berekend en NIET uit de POST-body
        // overgenomen. Anders zou deze handler een generiek herschrijf-eindpunt voor
        // willekeurige shortcode-attributen zijn (handboek sectie 8). Om dezelfde
        // reden wordt de meegegeven shortcode eerst getoetst: alleen een openingstag
        // van een element dat de responsive-opties daadwerkelijk kent, mag geraakt
        // worden.
        case 'set_responsive_breakpoints':
            $before = wp_unslash( $_POST['before'] ?? '' );
            if ( $before === '' ) {
                wp_send_json_error( 'before-waarde ontbreekt.' );
            }
            ...
            if ( $bp_theme === null ) {
                wp_send_json_error( 'Theme options bevatten geen bruikbare laptops-, tablets- en mobiles-breakpoint.' );
            }
            $post = get_post( $post_id );
            if ( ! $post ) {
                wp_send_json_error( 'Post niet gevonden.' );
...
function aspera_get_violation_admin_link( string $category, string $rule, $post_id, string $detail = '' ) {
    ...
    switch ( $category ) {
        ...
        case 'smtp':
            return [ 'url' => admin_url( 'admin.php?page=wp-mail-smtp' ), 'title' => 'WP Mail SMTP' ];
...
function aspera_get_rule_context(): array {
    ...
    $ctx = [
        ...
        'smtp_plugin_inactive' => [ 'label' => 'WP Mail SMTP niet actief', 'explanation' => 'Zonder WP Mail SMTP verstuurt WordPress via de PHP mail-functie van de server, zonder authenticatie en zonder verzendlog.', 'action' => 'Installeer en activeer WP Mail SMTP en configureer een mailer.' ],
        ...
        'smtp_from_name_not_forced' => [ 'label' => 'From Name niet geforceerd', 'explanation' => 'Zonder Force From Name bepaalt elke plugin zelf de afzendernaam, wat inconsistente afzenders in de inbox oplevert.', 'action' => 'WP Mail SMTP > Settings > Mail > zet "Force From Name" aan.' ],
...
function aspera_dashboard_widget_render(): void {
    ...
    echo '<style>
        ...
        @media print {
            html, body, body.wp-admin, #wpwrap, #wpcontent, #wpbody, #wpbody-content { background:#fff !important; }
    ...
    foreach ( $cat_labels as $key => $label ) {
        ...
        if ( $cat_err ) {
            ...
            if ( empty( $active_viols ) && $ignored_count > 0 ) {
                ...
                echo '<label class="aspera-select-all-lbl"><input type="checkbox" class="aspera-select-all-cb">&ensp;Bulk-select</label>';
                ...
                foreach ( $active_viols as $v ) {
                    ...
                    if ( $rule_action || $has_fix ) {
                        ...
                        if ( $has_fix ) {
                            ...
                            } elseif ( $fa === 'set_responsive_breakpoints' ) {
                                // Eigen tak: de generieke before/after-preview kapt op 80 tekens
                                // af en toont bij een lange lijst-shortcode alleen het begin,
                                // waar de breakpoint-attributen niet in staan.
                                echo esc_html( $fix['description'] ?? 'de drie breakpoints op de Aspera-standaard zetten' );
                            } else {
        ...
        'callback'            => function ( WP_REST_Request $req ) {
            ...
            foreach ( $post_ids as $post_id ) {
                ...
                foreach ( $data['data'] as $element_key => $element ) {
                    ...
                    // css kan als URL-encoded JSON-string opgeslagen zijn (Design-tab formaat);
                    // decode eerst zodat we een modern object niet als legacy string flaggen.
                    if ( is_string( $element_css ) && $element_css !== '' ) {
                        $decoded = json_decode( urldecode( $element_css ), true );
                        if ( is_array( $decoded ) ) {
                            $element_css = $decoded;
                        }
                    }
                ...
                if ( ! $publicly_queryable ) {
                    ...
                    if ( ! empty( $extra ) ) {
                        $post_violations[] = [
                            ...
                            'detail' => '"' . $slug . '" — publicly_queryable is false maar supports bevat: ' . implode( ', ', array_values( $extra ) ),
                        ];
                    }
                }
    ...
    register_rest_route( 'aspera/v1', '/site/audit', [
        ...
        'callback'            => function ( WP_REST_Request $req ) {
            ...
            $severity_map = [
                ...
                'smtp_plugin_inactive'                   => 'warning',
                'smtp_announcements_visible'             => 'warning',
                'smtp_dashboard_widget_visible'          => 'warning',
            ...
            // ── SMTP (WP Mail SMTP) ──────────────────────────────────────
            // De drie UI-opties horen aan te staan: geen announcements, geen
            // dashboard-widget, geen wekelijkse samenvattingsmail. Ze zijn
            // alleen te beoordelen als de plugin daadwerkelijk actief is.
            $smtp_violations = [];
            $smtp_plugins    = [ 'wp-mail-smtp', 'wp-mail-smtp-pro' ];
            $plugin_slug_set = aspera_get_plugin_slugs();
            $smtp_is_active  = ! empty( array_intersect( $smtp_plugins, $plugin_slug_set['active'] ) );

            if ( ! $smtp_is_active ) {
                $smtp_is_installed = ! empty( array_intersect( $smtp_plugins, $plugin_slug_set['installed'] ) );
                $smtp_violations[] = [
                    'rule'     => 'smtp_plugin_inactive',
                    'severity' => 'warning',
                    'detail'   => $smtp_is_installed
                        ? 'WP Mail SMTP is geinstalleerd maar niet geactiveerd'
                        : 'WP Mail SMTP is niet geinstalleerd',
                ];
            } else {
                $smtp_raw  = get_option( 'wp_mail_smtp', [] );
                $smtp_opts = is_array( $smtp_raw ) ? $smtp_raw : ( is_string( $smtp_raw ) ? maybe_unserialize( $smtp_raw ) : [] );
                $smtp_gen  = is_array( $smtp_opts ) ? (array) ( $smtp_opts['general'] ?? [] ) : [];

                $smtp_required = [
                    'am_notifications_hidden'       => [ 'smtp_announcements_visible',    'Hide Announcements' ],
                    'dashboard_widget_hidden'       => [ 'smtp_dashboard_widget_visible', 'Hide Dashboard Widget' ],
                    'summary_report_email_disabled' => [ 'smtp_summary_email_enabled',    'Disable Email Summaries' ],
                ];
                foreach ( $smtp_required as $opt_key => list( $smtp_rule, $smtp_label ) ) {
                ...
                // Mailer-specifiek: Brevo vereist een ingevulde API-key. De plugin
                // gebruikt intern nog de oude merknaam sendinblue als mailer-sleutel;
                // 'brevo' wordt meegenomen voor het geval die ooit hernoemd wordt.
                // De key wordt niet gevalideerd, alleen op aanwezigheid gecontroleerd.
                $smtp_mail_cfg = is_array( $smtp_opts ) ? (array) ( $smtp_opts['mail'] ?? [] ) : [];
                $smtp_mailer   = (string) ( $smtp_mail_cfg['mailer'] ?? '' );

                // Toegestane verzendpaden: Brevo of Other SMTP. De verificatie-
                // instellingen van Other SMTP (host, poort, encryptie, auth) blijven
                // bewust buiten scope.
                if ( ! in_array( $smtp_mailer, [ 'sendinblue', 'brevo', 'smtp' ], true ) ) {
                    $smtp_violations[] = [
                        'rule'     => 'smtp_mailer_not_allowed',
                        'severity' => 'warning',
                        'detail'   => 'mailer="' . ( $smtp_mailer !== '' ? $smtp_mailer : 'leeg' ) . '" — alleen Brevo of Other SMTP zijn toegestaan',
                    ];
                }

                if ( in_array( $smtp_mailer, [ 'sendinblue', 'brevo' ], true ) ) {
                    $brevo_cfg = is_array( $smtp_opts ) ? (array) ( $smtp_opts[ $smtp_mailer ] ?? [] ) : [];
                    if ( trim( (string) ( $brevo_cfg['api_key'] ?? '' ) ) === '' ) {
                        $smtp_violations[] = [
                            'rule'     => 'smtp_brevo_api_key_missing',
                            'severity' => 'critical',
                            'detail'   => 'Mailer staat op Brevo (' . $smtp_mailer . ') maar api_key is leeg — uitgaande mail wordt niet verstuurd',
                        ];
                    }
                }

                // Force-toggles gelden ongeacht de gekozen mailer.
                if ( empty( $smtp_mail_cfg['from_email_force'] ) ) {
                    $smtp_violations[] = [
                        'rule'     => 'smtp_from_email_not_forced',
                        'severity' => 'warning',
                        'detail'   => '"Force From Email" staat uit (from_email_force) — plugins kunnen een eigen afzenderadres meesturen',
                    ];
                }
                if ( empty( $smtp_mail_cfg['from_name_force'] ) ) {
                    $smtp_violations[] = [
                        'rule'     => 'smtp_from_name_not_forced',
                        'severity' => 'warning',
                        'detail'   => '"Force From Name" staat uit (from_name_force) — plugins kunnen een eigen afzendernaam meesturen',
                    ];
                }
            }
            $categories['smtp'] = [
                'violation_count' => count( $smtp_violations ),
                'violations'      => $smtp_violations,
                'error'           => null,
            ];
